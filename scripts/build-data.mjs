...
const entries = collections.flatMap((collection) => data[collection].map((entity) => ({id:entity.id,entityType:typeFor(collection,entity),collection,status:"active",firstPublishedIn:entity.firstPublishedIn ?? (collection === "genres" ? "0.3.1" : "0.3.0")}))).sort((a,b)=>a.id.localeCompare(b.id));
...
const workRows=data.works.map(w=>{const genres=(w.genreIds??[]).map(id=>genresById.get(id)).filter(Boolean);return [w.id,w.code,w.titles?.ja,w.releaseDate?.value,w.durationMinutes,(w.personRelations??[]).map(r=>peopleById.get(r.personId)??r.personId).join(";"),w.makerId,(w.genreIds??[]).join(";"),genres.map(g=>g.names?.ja).join(";"),genres.map(g=>g.names?.["zh-CN"]).join(";"),genres.map(g=>g.names?.en).join(";")]});
