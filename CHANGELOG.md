...
- 为 Pilot A 的 5 部作品建立首轮 Work–Genre 引用闭环。
- 只映射日文原题或现有来源明确支持的分类，不根据演员印象推测内容。
- 新增“初体験”和“性感開発”两个三语分类条目。
- 在 5 份 Work Source Record 中记录 Genre 映射依据。
- Registry 支持实体通过 `firstPublishedIn` 覆盖默认首次发布版本。
